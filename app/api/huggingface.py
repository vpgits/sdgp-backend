...
def generate_mcq(key_point_id, access_token, refresh_token):
    logging.info("Attempting to generate mcq")
    supabase_client: Client = get_supabase_client(access_token, refresh_token)
    response = supabase_client.table("key_points").select("context").eq("id", key_point_id).execute()
    text = response.data[0].get("context")
    payload = {
        "inputs": "### Instruction : You are given a sequence of text. Generate a logical question, correct answer "
                  "and three incorrect answers suitable for mcq from the input text.The output should be formatted as "
                  "a json in the below format." + "{\"type\": \"object\", \"properties\": {\"Output\": {\"type\": "
